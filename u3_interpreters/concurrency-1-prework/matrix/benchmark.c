...
int main(int argc, char *argv[]) {
  ...
  double naive_time, two_threads, four_threads, five_threads, ten_threads;
  ...
  flush_cache();
  gettimeofday(&start, NULL);
  parallel_matrix_multiply(C_parallel, A, B, n, n, n, 2);
  gettimeofday(&stop, NULL);
  two_threads =
      ((stop.tv_sec - start.tv_sec) * 1000000 + stop.tv_usec - start.tv_usec);
  ...
  flush_cache();
  gettimeofday(&start, NULL);
  parallel_matrix_multiply(C_parallel, A, B, n, n, n, 4);
  gettimeofday(&stop, NULL);
  four_threads =
      ((stop.tv_sec - start.tv_sec) * 1000000 + stop.tv_usec - start.tv_usec);
  ...
  flush_cache();
  gettimeofday(&start, NULL);
  parallel_matrix_multiply(C_parallel, A, B, n, n, n, 5);
  gettimeofday(&stop, NULL);
  five_threads =
      ((stop.tv_sec - start.tv_sec) * 1000000 + stop.tv_usec - start.tv_usec);
  ...
  flush_cache();
  gettimeofday(&start, NULL);
  parallel_matrix_multiply(C_parallel, A, B, n, n, n, 10);
  gettimeofday(&stop, NULL);
  ten_threads =
      ((stop.tv_sec - start.tv_sec) * 1000000 + stop.tv_usec - start.tv_usec);

  printf("%d,%.3f,%.3f,%.3f,%.3f,%.3f\n", n, naive_time / 1000000, two_threads / 1000000, four_threads / 1000000, five_threads / 1000000, ten_threads / 1000000);

  // printf("Naive: %.3fs\nParallel: %.3fs\nThread Count: %d\n", naive_time /
  // 1000000,
  //        parallel_time / 1000000, t);
